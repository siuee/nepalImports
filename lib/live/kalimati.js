...
/**
 * Kalimati serves an empty table until a dated POST with session + CSRF.
 * Flow: GET (cookies) → POST price form → parse returned HTML.
 */
export async function fetchKalimatiPrices() {
  ...
  let { heading, rows } = parseKalimatiHtml(html1);

  if (!rows.length) {
    const token = extractCsrf(html1);
    if (!token) throw new Error("Kalimati: missing CSRF token");
    ...
    const parsed = parseKalimatiHtml(html2);
