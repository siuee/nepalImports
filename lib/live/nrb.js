...
/**
 * NPR per 1 USD from a `fetchNrbForex()` result or `/api/live/forex` JSON body.
 * Uses official sell quote ÷ currency unit (USD is usually unit 1).
 */
export function nprPerUsdFromNrbForex(data) {
  if (!data || data.error || !data.rates?.USD) return null;
  const row = data.rates.USD;
  const q = Number(row.nprForQuote);
  if (!Number.isFinite(q) || q <= 0) return null;
  const unit = Number(row.unit) > 0 ? Number(row.unit) : 1;
  return q / unit;
}
