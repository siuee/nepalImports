...
export function inferCategoryFromNepali(nameNepali) {
  ...
  /* Tomato Nepali names include the sheep substring used in meat matching; classify before meat. */
  if (has(["गोलभेडा", "गोलभेंडा", "टमाटर"])) return "vegetable";
  ...
  /* Green peas — केराउ vs banana केरा */
  if (n.includes("केराउ")) return "vegetable";
...
export function resolveCommodityCategory(nameNepali, catalogEntry) {
  ...
  if (cc === "fruit") {
    if (
      ["fish", "meat", "dairy", "mushroom", "pulse", "root", "greens", "vegetable", "spice", "grain", "oilseed"].includes(
        inferred
      )
    ) {
      return inferred;
  ...
  if (cc === "meat") {
    if (["vegetable", "fruit", "greens", "root", "pulse", "spice", "grain", "oilseed", "mushroom"].includes(inferred)) {
      return inferred;
    }
    return "meat";
  }
...
export function buildCategoryTabs(items) {
  ...
  const ids = [...counts.keys()].filter((id) => id !== "all");
  ids.sort((a, b) => {
